...
随着业务的不断发展、与红布林合作的不断深入，同红布林一起开发了上门回收的流程，转转侧承担了上门回收师APP的开发。目前转转系 `App` 是一个典型的 `Hybrid App`，采用的是业内主流的做法：客户端内有大量业务页面使用 `WebView` 加载 H5 页面承载。
...
1、采用离线包、预渲染、预请求等技术进行优化，使页面达到秒开的效果，提高用户体验

2、使用JSBridge与客户端进行通信，完成商品标签打印和上门回收录音功能

3、集成UnoCSS，采用原子化方法进行CSS开发，提升项目的样式效率和可维护性

4、封装useSearch的hook，使用正则将用户输入进行分组，采用策略组提高匹配精度，通过原生for循环达到搜索的最佳效率，最后采用动态正则实现匹配结果高亮，直接使预约订单转化率提升3.46%

5、基于“多端多角色多状态多操作的页面设计”经验完成高质量、易维护的页面

6、沉淀跨部门沟通、跨公司合作开发、封闭开发的经验
...
UnoCSS是一个高性能且极具灵活性的即时原子化CSS引擎。它不同于传统的CSS框架，UnoCSS没有内置的核心工具类，所有的功能都是通过预设来提供的。UnoCSS采用的原子化CSS方法论，是将样式属性拆分成独立的、可重复使用的类，每个类定义一个样式属性，这种方式促进了模块化和可复用性



UnoCSS提供了一种更加轻量级且按需生成的方式来使用原子化CSS，而Tailwind CSS则提供了一个更为全面且预定义的工具类集合来帮助开发者构建界面。两者都有各自的优势，适用于不同的开发场景和偏好。
...
为了让客服高效联系客户，增加派单的数量。随着业务的发展，功能涵盖了对客服的管控和激励、对于上门回收的管控、线索数据的分析等，开发了线索管理、外呼统计、上门录音、仪表仪容、模拟登录等模块。
...
1、申请权限Code，接入RBAC权限管理平台；实现菜单权限，进一部封装useAccess实现按钮权限

2、实现了基于WebSocket的服务端推送功能，确保在接收到新线索时能够及时通知客服，并配备心跳检测机制以保持连接的稳定性。

3、利用ProComponents实现页面的快速搭建与高效开发；并自定义ProTable的列配置，扩展了valueType类型以满足特定数据显示需求

4、运用`@umi/max`的内置Model管理数据流，增强了状态管理和组件通信的效率

5、创建全局组件以集成第三方点控云外呼服务；通过调整Nginx配置实现iframe跨域问题的巧妙解决方案

6、推动后端协作开发模拟登录功能，极大地便利了客服问题排查流程，显著提升了解决问题的效率
...
1、负责整个项目的设计与开发，从零开始建立系统架构，包括域名解析和Nginx配置

2、对王者营地APP进行深入的网络抓包，获取关键接口并分析数据结构

3、构建了多样化的设备信息库，并通过动态token增强请求的有效性

4、通过动态版HTTP隧道服务器进行代理，以隐藏真实IP，并支持请求失败时自动重试

5、封装requestCounter中间件，并结合Redis实现每日数据请求量的统计分析

5、错误监控与预警：开发了错误上报机制，能够在请求失败时向企业微信发送通知，以便及时响应

6、接口安全: 封装了`checkDomain`中间件，确保只处理来自指定域名的请求，有效防止了非法访问

7、使用 PM2进行日志管理，包含了日志合并、分级、调整日志文件日期格式等，为快速定位和分析历史日志提供了便利
...
vscode插件开发 + vue +  naive-ui + element plus + axios + express + mysql + sequelize + redis + nginx

### 职责

1、主导项目的设计和开发，整个系统包含：vscode插件端、ChatGPT网页端、后台管理系统、API后端接口

2、在vscode 插件端使用内置接口registerCompletionItemProvider提供zzui组件的可选项，在按下enter时，使用命令调用导入的逻辑，完成自动use的操作。

3、在vscode 插件端使用createWebviewPanel创建普通webview，通过WebviewViewProvider.resolveWebviewView实现侧边栏的webview展示

4、在vscode 插件端封装Communication类，网页端封装vscodePlugin插件，实现了插件与Web端的高效通信

5、网页端使用naive-ui进行开发，支持多模型切换和手机验证码登录，提升了用户界面的安全性和易用性

6、API接口使用express进行开发，使用jwt进行鉴权，sequelize进行数据库操作，rateLimit进行限流，定时任务进行数据备份等，确保了系统的安全稳定运行。
