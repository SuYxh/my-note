...
const res = mergeArr(nums1, nums2);
console.log(res);
```
...
```js
function computedArr(arr1, arr2) {
    const result = []
    ...
    while (i >= 0 || j>= 0 || carry > 0) {
        const a1 = arr1[i] ? arr1[i] : 0
        const a2 = arr2[j] ? arr2[j] : 0
...
const arr1 = [1, 2, 3, 6];
const arr2 = [9, 8, 6, 7];
const arr3 = [1, 1, 1, 0, 3];

const res = computedArr(arr1, arr2);
console.log(res); 
```
...
const res = fn('aabbcccddeeee11223')
console.log(res)
```
...
function lengthOfLongestSubstring(str) {
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        if (result.str.includes(curStr)) {
            const diff = right - left
            if (diff >= result.length) {
                result.temp = result.str
                result.length = diff
            }
    ...
    if (result.str) {
        if (result.str?.length > result.temp.length) {
