...
function transition(promise, targetState, value) {
  if (
    promise.state === PROMISE_STATES.PENDING &&
...
function resolvePromiseWithValue(promise, x, thenableValues = []) {
  if (promise === x) {
    transition(
      promise,
      PROMISE_STATES.REJECTED,
      new TypeError("promise and x cannot refer to the same object.")
    );
  } else if (isPromise(x)) {
    if (x.state !== PROMISE_STATES.PENDING) {
      transition(
        promise,
        x.state,
        x.state === PROMISE_STATES.FULFILLED ? x.value : x.reason
      );
    } else {
      x.then(
        (value) => {
          ...
          (value) => {
            if (thenableValues.indexOf(value) !== -1) {
              transition(
                ...
                PROMISE_STATES.REJECTED,
                new TypeError(
                  "there is a thenable cycle that will lead to infinite recursion."
                )
              );
            }
            if (!isInvoked) {
