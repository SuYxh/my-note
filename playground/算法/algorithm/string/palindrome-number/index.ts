/**
 * @description: 查询 1-max 的所有对称数（数组反转）
 * @param {number} max
 * @return {*}
 */
...
/**
 * @description: 查询 1-max 的所有对称数（字符串前后比较）
 * @param {number} max
 * @return {*}
 */
...
/**
 * @description: 查询 1-max 的所有对称数（翻转数字）
 * @param {number} max
 * @return {*}
 */
...
// 数字反转
function reversal(n: number) {
  let rev = 0;
  while (n > 0) {
    console.log("n-1", n, n / 10, n % 10);
    rev = rev * 10 + (n % 10);
    n = Math.floor(n / 10);
    console.log("n-2", n);
  }
